...
def handle_vr_control(data):
    rospy.loginfo("data receive %s", data.data)
    for i in range (0, 6):
        buffer[i] = int(data.data[i])
        
def joystick_control():
    
    rospy.init_node('listener', anonymous=True)

    rospy.Subscriber('servo_vr_ros', String, handle_vr_control)
    stopFlag = True
    ack = True
    if joystick_count > 0:
        ...
        while True:
            ...
            if newValue == True and ack == True: 
                if debounce == 1: 
                    ...
                    string = '!' + string + '#'
                print('send: ', string)
                serialObject.write(bytes(str(string)).encode("utf-8"))
                ack = False
